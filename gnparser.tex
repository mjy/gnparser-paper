...
\begin{itemize}
  ...
  \item Scale and throughput. We create a name-string parser for particular needs of biodiversity aggregators. The core design requirement is to develop a lightweight library for massive inputs (millions of scientific names per second) processed locally.

  \item Stand-alone approach. A parser doesn't need access (neither locally nor remotely) to previously known information of genera, species, author names, other scientific names in a batch request, etc.\ \textit{gnparser} can only parse a string based on morphological features of limited amount of words: word letters case, stable nomenclature qualifiers (``\times'' for hybrid formula, ``ssp.'' for subspecies rank, ``var.'' for variance, etc.), digits for years, and their inter-dependencies.
