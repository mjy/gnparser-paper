...
approach very introspective and flexible. We believe that PEG suites our
...
\marginpar{Paddy: \\next line, specify which are the 'others' that are lacking
  (or not well served)}
...
programming language \cite{odersky2004overview}, and the Open Source \textit{Parboiled2} library \cite{parboiled2} which implements PEG in Scala. An alternative to \textit{parboiled2} is the Scala
...
modules (or subprojects in Scala terminology).
 ...
 % comment: can we include a box or Table with the syntax tree?
\begin{enumerate}
...
%comment PADDY START AGAIN HERE +612 9523 0835



  \item ``\textit{examples}'' contains  examples to assist developers in building \textit{parser} into other popular programming languages: Java, Scala, Python, Ruby, and R
  
  \item ``\textit{runner}'' contains the code that allows users to run ``\textit{parser}''
  from a command line as a standalone tool or to run it as a TCP/IP socket server. ``\textit{runner}'' depends on the ``\textit{parser}''.   The core part is the launch script
...
provided via a socket or file.
  ...
  \item ``\textit{web}'' contains a web application and a RESTful interface as a simpler method to access ``\textit{parser}''. It is a Play Framework \cite{wampler2011scala} application. It
...
simple HTML and REST API interfaces
\end{enumerate}
...
\begin{itemize}
  \item A \textit{parser} artifact is provided via Maven central repository of Java code (REF!). Physically
    it is a relatively small jar file without external dependencies. The
    artifact can be accessed by a build system such as Maven, Gradle, or SBT in
    other projects. The build system then identifies and provides access to any other required
    code-dependencies.

  \item A Zip-archived ``fat jar'' is located at the project's GitHub repository.
  ...
  \item The project's Docker container is located at Docker hub  (*REF!). Docker
    ...
    Scala and SBT -- can be run with simple commands that will, for example,  pull the Docker image from the Hub, and run the socket or web server on a desired port.
...
%comment: in the pdf, this 'Figure' appears before the two tables above, even though in the latex they are the other way round. Please check the journal requirements, but Figure legends usually follow the Figures.  and that 'Figure' is really one Table and one Figure.  I think they should be separated.
...
% box, and indents for nested elements?
% Dima: I will try to add another table with explanations
% http://globalnames.org/schemas/gnparser.json
