...
\begin{abstract} % abstract
  ...
  We will be able to investigate biology in new and different ways and on grander scales when we can integrate biological data from multiple sources. An obvious solution lies in using the scientific names of organisms as metadata. There are impediments to this because there may be many
  names for a taxon, or one name may apply to different taxa.  Names are represented by name-strings (a combination of characters, spaces and punctuation that represent the name). Within these, the same names may be spelled differently, be abbreviated, be annotated, or include or exclude authors of names and dates of establishment. We can minimize mismatches caused by such differences if we divide the name-strings into their component parts and establish the roles of each part that is, subject them to 'semantic parsing'. Once done, we can match entries on the most commonly used elements of names and/or set aside the elements that differ.

  \parttitle{Results}
  We introduce Global Names Parser (\textit{gnparser}), a tool based on the
  Parsing Expression Grammar algorithm. The parser is applied to scientific
  name-strings of any complexity. It assigns a semantic meaning (such as genus name, species epithet, annotation, rank, year of publication, names of authors, annotations, etc.) to all elements. Global Names Parser is written in Scala, a
  ...
  bioinformaticians and biologists working with large numbers of scientific names. It can replace expensive and
  error-prone manual parsing and standardization of scientific names in many
  situations and quickly increases the interoperability of distributed biological information.
  
  % delete from less than 15\% to greater than 85\% by improving the matches among variant forms of the same scientific names.

%Paddy comment - I think you should delete the last sentence.  We do not have data in this paper which provide information on levels of improvement.  In the challenges paper, we showed that name matching between pairs of sources improved to 85-100%, but this still doesn't give us a figure of content matching.  I am not sure what metric we would want to use for that.
...
\marginpar{Paddy: \\The start of Results and Discussion (of results) began with prior art and reasons for selecting SCALA. In my view, these parts should be in the Introduction and M&M.  So, I moved them. Please confirm that it reads OK}
\section*{Conventions}
...
without difficulties, understand why parser is important for them, give paper to
programmers}
...
%Comment paddy, ref needed above
...
following reasons:
c%comment Paddy: what does Synthax mean? Is it supposed to be Syntax? I changed
\begin{itemize}
...
\begin{itemize}
    ...
    language
\end{itemize}
%comment Paddy, next line, specify which are the 'others' that are lacking (or not well served)
...
%Comment Paddy.  What are the performance and memory problems.  You need a reference to defend this statement.  If you are not willing to do this, just remove the sentence about combinators. It is sufficient to say that the chosen package meets all of the needs. 
...
%comment Paddy. Can you tell us biologists what a grammar is, and more importantly why it is BAD to have an external grammar generator. The reasoning that explains your statement is not given (yet).
...
%Comment Paddy: It is not clear what Parboiled or 'it' are in the previous sentence. I think you will probably require several sentences.
...
%Comment paddy synthax or syntax, I changed to syntax? None of these parts will be comprehensible to a biologist. The issue may just be of English expression. It is not clear why there is a block of text and a series of dotpoints. Could it be changed simply to a list of dot points with some explanation associated with each point.  This will need more work.
...
%comment Paddy: It is not clear what 'to other parts' means. Again, special terms are listed here without any explanation of what they mean (or how they are achieved - how do you achieve 'language interoperability' for example.
...
%Comment Paddy: Again confused.  This section begins with 'gnparser', then subdivides that into 4 subprojects, then starts to describe one subpart, and going into a further level of components, and now this paragraph jumps back to the top.  
...
\marginpar{Paddy:\\ Surely the names came from GNI and not from resolver?}
...
\marginpar{Paddy:\\ how do you get percentage from this? Ditto below}
...
\marginpar{Paddy:\\ by what mechanism were the 'about 1000 name-strings discarded}
...
\marginpar{Paddy:\\ I think the Results and Discussion is not well balanced.  1. The first 6 paragraphs should be in the Introductory section.  2. This section should start with the results, and then go on to the discussion and justification of the approach.  I have made the changes, and hope they don't cause too much upset}
...
\marginpar{Paddy:\\ Two quality control issues spring to mind.  The use of 'golden' names list to cross-check names (that would have been able to weed out the GBIF errors), and the second is the engagement of the community to vet names - ideally as they encounter them in other contexts - such as through the use of tools like name-spotter}
...
%comment paddy: In my view, the ranking is arbitrary, so assuming in both cases convoluta is a terminal taxon, then the two strings most likely point to the same 'concept'. I regard knowling the rank is unimportant
...
%Comment paddy:  Biologists will not know the conventions of JSON, so it would be useful to indicate what all of bits of the output mean. Something like
...
%Comment paddy:  Biologists will not know the conventions of JSON, so it would be useful to indicate what all of bits of the output mean. My view is to repeat the result, with the meaning in some special way. For example with a box, and indents for nested elelements?
