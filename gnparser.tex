...
implemented as regular expression \cite{aho1992foundations}. Formally regular
languages over an alphabet $\Sigma$ are defined recursively as follows:
...
Kleene's theorem \cite{yu1997handbook}. For example, since finite automata
does not have memory, both languages are not regular: $ \{a^nb^n\,\vert\; n\ge
0\} $ and the language of arithmetic expressions. Approaches to names parsing
built on regular expressions \cite{gbif} can't express theoretically and deal
very hard practically with recursively nested data structures (and complex
scientific name strings are recursive by nature, as it is most obvious with
hybrid formulae). Narrow information of parsing context make it hard to
...
drawback is that it is a interpreted dynamic language with originally
single-threaded runtime in virtual machine. We needed to move forward to an
...
fulfil requirements: Java virtual machine and Scala programming language
\cite{odersky2004overview}, and parboiled2 library \cite{parboiled2} (Scala
parser combinators \cite{moors2008parser} library has known performance and
...
\begin{itemize}
  ...
  programming languages: Java, Scala, Python, Ruby, R
...
``\textit{gnparser}'' [TODO: link to extensions].

\subsection*{parser}

``\textit{parser}'' contains all the guts for successful parsing of a
scientific name: parsing grammar, AST for a scientific name, warnings and
errors facilities, formatters (normalizer, canonizer, JSON
\cite{bray2014javascript} renderer). Typical usage is as external library to
provide parsing facilities: language interoperability, command line tool,
web/socket-servers, etc.

``\textit{gnparser}'' accepts $String$ as input, and returns a $Result$.
$Result$ then can be formatted to JSON that contains useful information as
follows:
...
\begin{itemize}
  ...
  description) of provided name [TODO: documentation]
...
of command line tool and socket server. Core part is the launch script
``\textit{gnparse}'' (both for Linux/Mac and Windows) that creates JVM
instance and run ``\textit{parser}'' on multiple threads against the input
