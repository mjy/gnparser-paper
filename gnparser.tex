...
\begin{abstract} % abstract
  ...
  \parttitle{Background} Scientific names in biology are the universal link that allows to integrate information about organisms from a vast variety of sources. The integration of data is even more critical now when biology is entering the ``Big Data'' age. There are impediments to such aggregation because names are often spelled with variations. Sometimes scientific names are abbreviated, annotated, or misspelled. Authorship is a part of a scientific name but the information about the authors may vary dramatically. To match all varieties of a name for the same taxon we need to divide them into their elements and classify each element according to its role. We refer to this as `parsing' the name.  Parsing significantly improves the matching of different spellings for the same species. It emphasises the most stable elements of names and then allows to fine-tune matched results using the remaining elements.
...
The authors thank David Mark Welch (Josephine Bay Paul Center, Marine Biological Laboratory) for the leadership at the beginning of this project. The authors thank administrators of the Species File Group for the much needed support during the transfer of the GNA grant from the Marine Biological Laboratory to University of Illinois.
