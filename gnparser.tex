...
authorship is expensive, slow, inflexible.  To scale this exercise up and to address such edge cases requires an algorithmic solution, a scientific name parser! \todo{scientific name parser do what?}

The strategy of the algorithmic approach is to identify which combinations of the most atomic elements of a name-string (i.e. the UTF-8 encoded characters) represent words (such as Genus name, species name, Authors, annotations \todo{why some are capitalized?}) or dates.  An early algorithmic approach to parsing scientific names was with regular
...
%comment paddy, two references required below
  \item A \textit{parser} artifact is provided via the Maven central repository of
    Java code (REFERENCE REQUIRED! \todo{paste href to}: \href{http://search.maven.org/#search%7Cga%7C1%7Cglobalnames}{Maven Cental}). Physically it is a relatively small jar file without
...
JSON output (\todo{should it be formatted as follows?}):
...
Jython, JRuby, R, JavaScript \todo{JavaScript? How? We could compile gnparser with scala.js that produces JavaScript, but it is not done so far} and a variety of other languages based on
Java Virtual Machine. If a user wants to use \textit{gnparser}  in some non-JVM
language s/he \todo{legitimate? :)} can connect to the parser via a socket server interface. There is
...
\begin{description}
  ...
  \item[Project home page:] https://github.com/GlobalNamesArchitecture/gnparser \todo{should be a href?}
...
DYM \todo{the abbreviation is not expanded and should be clear from the context?} and AAM designed gnparser. DYM created requirements and test suite. AAM
