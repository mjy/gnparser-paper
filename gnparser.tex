...
\begin{abstract} % abstract
  ...
  Modern biology would be unworkable without scientific names based on Linnaean
  nomenclature. Names of organisms are pervasive in documents, databases, and
  collections. They are metadata which allow us to discover, communicate, and
  combine information in biodiversity, ecology, molecular biology, medicine and
  many other fields. Indexing and organizing such information via scientific
  names is challenging for several reasons. Names for the same species may
  exist in a variety of alternative forms (name-strings) such as \textit{Aedes
  ...
  variation, and allowing matching on the same key parts of the name-string
  that make up a canonical form (``\textit{Aedes thurmanae}'' in the case of
  the examples above).
  \parttitle{Results}
  ...
  matches among variant forms of the same scientific names.
\end{abstract}
...
(letters, numbers, punctuation, spaces, symbols). The term ``name'' is one or
several words which points to a taxon. A name can be expressed by many
name-strings, some are well-formed and code compliant (in which case they are
regarded as scientific names, for examples see Table~\ref{table:carex}). There
many more names and surrogates for names that are not code-compliant and so are
not scientific.  There are tens of millions of scientific names and possibly
billions of legitimate name-strings.
...
plants (ICNCP) \cite{ICNCP}, Phylocode (ICPN) \cite{ICPN}. Conventionally, the
term 'scientific' refers to names that are compliant with any of the codes
excepting the Phylocode.
...
algorithmically deduce the semantic meaning of elements (words, annotations,
author information, date, etc.) within the name-strings.
...
vision of a ``Big New Biology'' in which information about species is
interconnected via scientific names. This problem can be addressed in part by
promoting obligatory standard names such that authors, editors and publishers
take responsibility for representing a scientific name in the same way.
% Paddy comment: reference, such as Zermoglio, or Boyle et al
The alternative is to apply reconciliation in which an infrastructure maps
alternative names for the same taxon together.  The former strategy would
require a costly international coordination, would not eliminate future human
mistakes, it cannot be applied to older documents, does not allow for multiple
points of view, nor for changes in taxonomic perspective. That is, even the use
of standard names requires reconciliation, and reconciliation can benefit from
standardization by resolving variant names to the recommended standard names.

The reconciliation process involves linking all known name-strings for the same
taxon into reconciliation groups. Membership include lexical variants of any
relevant name-strings, synonyms, surrogates, common names, and identifiers.
The assembly of lexical groups (various presentations of the same name or other
...
finds the 'common denominator' of spelling variants of scientific names – the
canonical form comprised only of the scientific words, (\textit{Carex
scirpoidea convoluta} in case of Table~\ref{table:carex}). It identifies other
important for reconciliation information (infraspecific ranks, original and
combination authorities etc.), and manages annotations and other intrusions
into the name-string. Lexical groups then require another level of folding into
reconciliation groups where homotypic and heterotypic synonyms are placed
together. This process benefits from access to accurates database of
...
Reconciliation relies heavily the most stable component of a name-string – a
canonical form. Such approach brings us to another problem – the same canonical
form may be used for more than one taxon or concept \cite{Remsen2016},
homonyms, chresonyms, or ranks of infraspecies. Analysis of ranks and the
authors found by a parser helps to separate homonyms chresonyms and other
similar names from each other, while an infrastructure to manage concepts is
unlikely in the forseeable future \cite{Patterson:inpress-a}.

After reconciliation, the final step is to provide an option to replace
outdated or incorrect names with ones that are endorsed by one or more
taxonomic authorities. This is referred to as resolution. Resolution requires
up to date high quality ``Taxonomic Authority Files'' \cite{VandenBerghe2015},
such as endorsed checklists (e.g. \cite{Zermoglio2016}), taxonomic sources
(e.g.  \cite{Boyle2013}) and taxonomic compilations  (e.g Catalogue of Life
...
from many different sources. The name-strings in the distributed sources may be
inconsistent in their format, impeding data integration. Impediments also
...
not code compliant names that may refer to organisms which are not formally
described taxa. When parsed, many such name-strings can be labelled as not
being scientific names and will therefore need special attention.  If a
name-string cannot be ingested by a high quality parser, the string is almost
certainly not a well-formed scientific name.  Common names are much less useful
in interconnecting distributed information \cite{Patterson:inpress-a}.
...
We believe a scientific name parser needs to perform well according to the following criteria.

\begin{enumerate}

  \item \textbf{High quality of semantic breakdown.} A parser should be able to
    break names into their semantic elements to the same standards that can be
    achieved by a  trained nomenclaturalist, or better. Success with this
    criterion will  overcome  problems with the different forms of names in
    various databases so that the research community is not limited to the use
    of databases where names have been parsed by humans or to exact string
    matching. High quality semantic parsers eliminate the need for humans to
    perform this very expensive, tedious and error-prone task.

  \item \textbf{Scope.} A parser should be able to parse all types of
    ...
    Without such scope, the matching of the same name in different sources will
    be incomplete and valuable information attached to scientific names is
    hidden from researchers.

  \item \textbf{Parsing Completeness.} All information included in a
    name-string may important. In addition to the elements of the canonical
    form, authorship, year, or rank information may allow us to identify or
    disambiguate homonyms, similar names, synonyms, spelling mistakes, or
    chresonyms. A good parser designed for scientific names allows people who
    manage large collections of names, or biodiversity databases that include
    names,  to weed out mistakes and increase the quality of their databases.

  \item \textbf{Speed.} For the purposes of a names-based cyberinfrastructure
    (the vision of the Global Names Architecture), a parser needs to be fast
    and handle periodic heavy loads if it is to offer real time services to
    usesr. High throughput of parsing becomes very important when managing
    federated data environments. Increased speed with which people will receive
    requested information improves user satisfaction, and it reduces the costs
    of dependent projects in both hardware and software development.

  \item \textbf{Accessibility.} Our commitment to open source is most
    cost-effective when the tools are available to the widest possible
    audience. A parser needs good documentation, the ability to work as a
    library, act as a command line tool, as a tool with graphical interface,
    and to run as socket and RESTful services.
...
\marginpar{Paddy comment.\\  I am not confident that the coverage of prior art is
complete.  Until reminded, FAT was excluded.  I recollect WoRMS had some
tool, Thomson Ecological had some tool, George Garrity had some
commercialized product. I suspect all authors should try to find any and all
tools that work with biological names parsing.\\
Dima:\\
WoRMS use our parser, FAT seems in camp with TaxonFinder, NetiNeti.  VandenBerghe2015 paper
lists ours, theirs, and http://bsbidb.org.uk/taxonnameparser.php which is
pretty horrible and not a project to download.}


Until recently the problem of parsing scientific names  has been addressed by
...
% paddy comment. When a statement like this is made, references need to be
% included.  How else do we know if the statement is correct.
\
...
informatics.\marginpar{Paddy:\\Perhaps list some teams who uses
  parser.\\Dima:\\Noone uses gnparser yet, except us. We talk about use of
biodiversity parser in Background}
