...
There is an explosion of algorithms and tools in machine learning and natural language processing and collection of approaches for recognizing parts of texts is growing rapidly. For example there are statistical parsing[?], strict and fuzzy context-free grammars[?], named entity recognition[?]. Unsupervised deep learning in particular looks very interesting in ability to increase quality of entity recognition without extensive curation and programming efforts by a human. At the moment the following design goals influence our decision do not move in these directions for parsing scientific name-strings.

\begin{itemize}
  \item Scale and throuput. We need a name-string parser for very particular needs of biodiversity aggregators. The core design requirement is to develop a lightweight library for massive inputs (millions of scientific names per second) processed locally.
...
\begin{verbatim}
    Crataegus chlorosarca subtaxon pubescens E.L.Wolf
      (could not parse two instances of intraspecific epithets)
