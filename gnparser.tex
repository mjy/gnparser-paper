...
Data for our tests were 1000, 100,000 and 1,000,000 name-strings randomly
chosen from 24 million names of the Global Names Index. These datasets consist
of name-strings acquired from a variety of data sources and are a mixture of
well-formed names, names with formatting and spelling mistakes, and non-name
strings misused as names.

We compared performance of gnparser with 2 other projects -- Biodiversity
parser (also developed by Global Names), and GBIF name-parser. Another parser
we considered was YASMEEN parser from iMarine project\cite{VandenBerghe2015},
however we found that on our dataset it generated two orders of magnitude more
mistakes than other parsers, and we decided to exclude it from the tests.

To find out the throughput of parsing we used a computer with Intel i7-4930K
CPU (6 cores, 12 threads, at 3.4 GHz), 64GB of memory, and 250GB
Samsung 840 EVO SSD, running Ubuntu version 14.04. Each parser processed
1000000 name-strings.

For determining precision and recall dataset size was 1000 names. For
determining false negatives accuracy dataset size was 100,000 names.  We
decided to concentrate on the most common denominator of output from the
parsers -- canonical form, and terminal aurhorship. Canonical form represents
the most stable parts of the name, while terminal aurhorship corresponds to
authors of the lowest element of the name.

For precision and recall we picked combination of canonical form and
authorship as criteria. When they are determined correctly we get true
positive \textit{tp}, when they are determined in a non-name string -- it is
false positive \textit{fp}. Correctly found non-parseable strings are true
negatives \textit{tn}, false negatives \textit{fn} which faild to be parsed
correctly. We determined canonical form and terminal authorship by human
driven pattern matching. If name-strings contain spelling mistakes but
canonical form and/or terminal authorship could be extracted by human, we
considered them as theoretically parseable. Name strings had to correspond to
common format of scientific names, but not to be really valid names.
Validation of names is out of scope of parsers.

Precision is a percentage of name that been parsed correctly and is calculated
as \(Precision = tp / (tp + fp)\). Recall is a percentage of correct items
found and is calculated as \(Recall = tp / (tp + fn)\). \textit{F1 measure} is
a balanced harmonic mean (where Recall and Precision have the same weight) and
is calculated as \(F1 = 2 * Precision * Recall /(Precision + Recall)\)

Accuracy for unparsed results had been calculated as \(Accuracy =
fn/(tn+fn)\).This accuracy shows how many names that should had been parsed
were not parsed. We processed 100 000 random name-strings by each parser and
found by hand which unparsed resultes were false negatives. Also we looked at
names uniquely unparsed by each program and collected them in
unparsed-comparisons.txt file.
...
\begin{table}[htb]
  \begin{center}
    \caption{Precision/Recall Results}\label{table:precision}
    \begin{tabular}{|l|*{3}{l}|}
      \hline
      ...
      \textit{False Positive}& 11       & 33          & 16           \\
      \textit{False Negative}& 0        & 0           & 0            \\
      \textit{Precision}     & 0.9888551& 0.966599    & 0.9837893    \\
      \textit{Recall}        & 1.0      & 1.0         & 1.0          \\
...
\begin{table}[htb]
  \begin{center}
    \caption{Acuracy of unparsed results, out of 100 000
    name-strings}\label{table:unparsed}
    \begin{tabular}{|l|*{3}{l}|}
      \hline
                             & gnparser & gbif-parser & biodiversity \\
      \hline
      \textit{Total unparsed}& 1131     & 1082        & 1161         \\
      \textit{True Negative} & 1129     & 940         & 1152         \\
      \textit{False Negative}& 2        & 142         & 9            \\
      \textit{Accuracy}      & 0.998231 & 0.868761    & 0.9922481    \\
      \hline
    \end{tabular}
  \end{center}
\end{table}
