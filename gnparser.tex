...
Next logical step was to model scientific names by Type-3 grammar type from
formal languages theory -- regular grammars
[\href{https://en.wikipedia.org/wiki/Chomsky_hierarchy}{Chomsky hierarchy}].
Such grammar constrains production rules to as follows: $A\rightarrow a$ and
$A\rightarrow aB$. Particularly, regular grammars (that theory lies behind
"Regular expressions") can't have recursive definitions of rules. Approaches
built on regular expressions (gbif) deal very hard with with recursive
patterns (and complex scientific name strings are recursive by nature, as it
...
Broader class of languages is generated by Type-2 grammar type -- context-free
grammars (CFG). Grammars of that type are general enough to recognize most of
programming languages. Particularly, it can handle recursive parts of a
scientific name described earlier. In view of rare cases of ambiguities in
scientific names parts (???: forma, subspecies in parentheses) we chose even
simpler formal grammar -- parsing expression grammar. It it similar to CFG
with one difference: the choice operator selects the first match in PEG, while
it is ambiguous in CFG. Although, we did not lose generality as syntax of PEG
very close to CFG syntax; semantics of parsing engine could be improved to
handle ambiguities.
...
implementation. As expected, PEG is well suited for recursive texts with
formally defined grammars. Linnaean taxonomy is essentially a tree-based
classification. Scientific names follow rules of nomenclatural codes, and
therefore order and capitalization of elements in scientific names are well
structured.
...
In 2015 we decided to use everything we learned, and found triple of
technologies to fulfil requirements: Java virtual machine, Scala programming
language, and parboiled2 library.
...
We write ``\textit{gnparser}'' -- completely
new parser in Scala to achieve significant boost in speed, scalability and
portability of the library. In this manuscript we describe features,
...
parboiled2 is open sourced that allowed us to extend it to
needs of ``\textit{gnparser}''.
...
\begin{itemize}
  ...
  \item $quality\_warnings$ describe potential problems (level and description)
  of provided name [??? doc]
...
m/GlobalNamesArchitecture/gnparser/blob/master/JSON_FIELDS.md}{link}]. JSON schema can be found at [??? link].
...
both with simple HTML [??? picture] and REST API interface.
