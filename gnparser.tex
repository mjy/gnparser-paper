...
\begin{abstract} % abstract
  ...
  collections. They are metadata which allow us to discover, communicate, and combine information in
  biodiversity, ecology, molecular biology, medicine and many other fields. Indexing and organizing such information via scientific names is
  ...
  variation, and allowing matching on the same key parts of the name-string that make up
...
(letters, numbers, punctuation, spaces, symbols). The term ``name'' is one or several words 
which points to a taxon. A name can be expressed by many name-strings, some are
well-formed and code compliant (in which case they are regarded as scientific names, for examples see
Table~\ref{table:carex}). There many more names and surrogates for names that are not code-compliant and so are not scientific.  There are tens of millions of scientific names and possibly billions
of legitimate name-strings.
...
plants (ICNCP) \cite{ICNCP}, Phylocode (ICPN) \cite{ICPN}. Conventionally, the term 'scientific' refers to names that are compliant with any of the codes excepting the Phylocode.
...
algorithmically deduce the semantic meaning of elements (words, annotations, author information, date, etc.) within
the name-strings.
...
\begin{table}[!htb]
  ...
  \caption{Some legitimate versions of the scientific name for the Northern
    Bulrush or Singlespike sedge.  The genus (\textit{Carex}), species (\textit{scirpoidea}),
    and subspecies (\textit{convoluta}) may be annotated (var. subsp., and ssp.) or
...
vision of a ``Big New Biology'' in which information about species is interconnected via scientific names. This
...
scientific name in the same way. 
% Paddy comment: reference, such as Zermoglio, or Boyle et al
The alternative is to apply reconciliation in
...
The reconciliation process involves linking all known name-strings for the same taxon into
...
finds the 'common denominator' of spelling variants of scientific names – the canonical form comprised only of the scientific words,
...
Reconciliation relies heavily the most stable component of a name-string – a canonical
...
from many different sources. The name-strings in the distributed sources
may be inconsistent in their format, impeding data integration. Impediments also
...
not code compliant names that may refer to organisms which are not formally described taxa. When parsed, many such name-strings
can be labelled as not being scientific
names and will therefore need special attention.  If a name-string cannot be
...
\begin{enumerate}
  ...
  \item \textbf{High quality of semantic breakdown.} A parser should be able to break names into their
    semantic elements to the same standards that can be achieved by a  trained
    nomenclaturalist, or better. Success with this criterion will  overcome  problems
    with the different forms of names in various databases so that the research community is not limited to
    the use of databases where names have been parsed by humans or to exact string
    matching. High quality semantic parsers eliminate the need for humans to perform
  ...
  \item \textbf{Scope.} A parser should be able to parse all types of
    ...
    Without such scope, the matching of the same name in different sources will be
  ...
  \item \textbf{Parsing Completeness.} All information included in a
    name-string may important. In addition to the elements of the canonical form, authorship, year, or
    rank information may allow us to identify or disambiguate homonyms, similar names, synonyms,
    spelling mistakes, or chresonyms. A good parser designed for scientific names allows people who manage large
    collections of names, or biodiversity databases that include names,  to weed out mistakes and increase the quality of their
    databases.

  \item \textbf{Speed.} For the purposes of a names-based cyberinfrastructure (the vision of the Global Names Architecture), a
    parser needs to be fast and handle periodic heavy loads if it is to offer real time services to usesr. High throughput of parsing becomes very important when managing federated data
    environments. Increased speed with which people will receive requested
    information improves user satisfaction, and it reduces the costs of
    dependent projects in both hardware and software development.

  \item \textbf{Accessibility.} Our commitment to open source is most cost-effective
...
% Paddy comment.  I am not confident that the coverage of prior art is complete.  Until reminded, FAT was excluded.  I recollect WoRMS had some tool, Thomson Ecological had some tool, George Garrity had some commercialized product. I suspect all authors should try to find any and all tools that work with biological names parsing.
...
% paddy comment. When a statement like this is made, references need to be included.  How else do we know if the statement is correct.
\
