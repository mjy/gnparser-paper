...
than other parsers (Precision 0.534, F1 0.6962), and we decided to exclude it
from the tests.
...
negatives \textit{tn}, false negatives \textit{fn} name strings a `valid
scientific names` which completely failed parsing. We determined canonical
form and terminal authorship by human driven pattern matching. If name-strings
contain spelling mistakes but canonical form and/or terminal authorship could
be extracted by human, we considered them as theoretically parseable. Name
strings had to correspond to common format of scientific names, but not to be
really valid names. Validation of names is out of scope of parsers.

Precision is a percentage of names that been parsed correctly and is
calculated as \(Precision = tp / (tp + fp)\). Recall is a percentage of
correct items found and is calculated as \(Recall = tp / (tp + fn)\).
\textit{F1 measure} is a balanced harmonic mean (where Recall and Precision
have the same weight) and is calculated as \(F1 = 2 * Precision * Recall
/(Precision + Recall)\)

Precision for results where parsing failed was calculated from 100 000
dataset. Approximately 1000 names out of 100 000 had been discarded by the
parsers. Precision in this case shows percentage of correctly discarded names.
We do not know recall, as it was not feasible to manually find it for 100 000
names. To get a glimpse on names which had to be discarded, but were parsed
instead we analysed differences in failed parsing results between 3 parsers.
